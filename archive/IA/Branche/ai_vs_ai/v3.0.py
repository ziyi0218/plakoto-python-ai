...
def simulateur(
    ...
    """
    完全随机模拟：两名 IQ=0 玩家交替随机走法直到：
      - 当前玩家满足 check_victoire -> 返回该玩家 "B" 或 "N"
      - 超出 max_turns 回合 -> 返回 None（平局）
    """
...
# 执行一个宏走法序列，并消耗对应的骰子
# ------------------
def apply_macro_move(
    board: Dict, 
    seq: List[Tuple[int, int, int]], 
    joueur: str, 
    dice: List[int]
) -> None:
    for origine, dest, d in seq:
        deplacer_pion(board, origine, dest, joueur)
        if d in dice:
            dice.remove(d)


# ------------------
# 蒙特卡洛选择：对每个候选宏走法用 simulateur 模拟 ns 次
# ------------------
# 顶层可 picklable helper
def _worker_seq(args):
    """
    单个任务：在一份拷贝的 board 上
    应用某条宏走法 seq，然后做 ns 次随机 simulateur，
    返回本 seq 的胜利次数。
    args = (board, joueur, dice, seq, ns, sim_max_turns)
    """
    board, joueur, dice, seq, ns, sim_max = args
    wins = 0
    for _ in range(ns):
        sim_board = copy_plateau(board)
        sim_dice  = dice.copy()
        # 先应用这个宏走法
        for o, dest, d in seq:
            deplacer_pion(sim_board, o, dest, joueur)
            if d in sim_dice:
                sim_dice.remove(d)
        # 随机模拟下一步由对手开始
...
def monte_carlo_select(
    ...
    ns: int,
    sim_max_turns: int = 200,
    n_procs: int = None
) -> List[Tuple[int, int, int]]:
    """
    并行蒙特卡洛：
      - 每个候选宏走法只启动 1 个进程，在子进程内完成 ns 次 simulateur，
      - 进程个数 = len(candidates)（而不是 len(candidates)*ns），
      - 轻拷贝 + IPC 开销大幅下降。
    """
    ...
    # 准备每个子任务的参数包
    tasks = [
        (board, joueur, dice, seq, ns, sim_max_turns)
        for seq in candidates
    ]

    with ProcessPoolExecutor(max_workers=n_procs) as exe:
        # exe.map 会顺序返回每个 seq 对应的胜利次数
        for seq, wins in zip(candidates, exe.map(_worker_seq, tasks)):
            # 更新最优
...
def match_ai_vs_ai(
    ...
    """
    两个 AI 轮流落子：
      - 先手用 ns1 次模拟选步，后手用 ns2 次模拟。
      - 每回合：掷骰 -> 生成所有宏走法 -> 蒙特卡洛选最优 -> 执行 -> 检胜利 -> 换手
      - 超过 max_turns 未分胜负返回 None（平局）
    """
    board   = copy_plateau(plateau_init)
    current = joueur_init
    ...
    for _ in range(max_turns):
        ...
        apply_macro_move(board, best_seq, current, dice)
...
for i in range(100):
    ...
    winner = match_ai_vs_ai(plateau, "B", ns1, ns2, max_turns=200)
