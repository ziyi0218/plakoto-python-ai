...
# 4 avril Mahdi (v1 IA Monte Carlo)
# code des fonctions et redéfinition du nouveau main
...
def simulateur(
    ...
    """
    Simulateur random pour IQ=0: deux joueurs jouent aléatoirement jusqu'à
    - un joueur satisfait check_victoire -> retourne "B" ou "N"
    - max_turns atteint -> None (match nul)
    """
...
def simulation_worker(args):
    ...
    if board_copy is None:
        return 0
    next_player = "B" if current_player == "N" else "N"
    result = simulateur(board_copy, next_player)
    if result is None:
        return 0  # mettre à jour en 0.5 si on veut demi-point
    return 1 if result == current_player else 0

# ------------------ EVALUATION MONTE CARLO DES MACRO COUPS ------------------
def monte_carlo_best_move(
    ...
    macro_moves = generate_macro_moves(board, dice, current_player)
    candidate_moves = [m for moves in macro_moves.values() for m in moves]
...
def jouer_partie_ia_vs_ia(
    ...
    while turn_count < max_turns:
        ...
        best_macro = monte_carlo_best_move(
            plateau,
            dice,
            current_player,
        ...
        if verbose:
            print(f"Tour {current_player} | Dés: {dice} -> {best_macro}")
        if best_macro:
            for origine, destination, _ in best_macro:
                deplacer_pion(plateau, origine, destination, current_player)
        if check_victoire(plateau, current_player):
            return current_player
        current_player = "B" if current_player == "N" else "N"
        turn_count += 1

    return "NUL"

# ------------------ BENCHMARK ------------------
def benchmark_ia_vs_ia(nb_parties: int, sim_b: int, sim_n: int):
    start = time.time()
    victoires_b = 0
    victoires_n = 0
    ...
    for i in range(nb_parties):
        t0 = time.time()
        gagnant = jouer_partie_ia_vs_ia(sim_b, sim_n)
        t1 = time.time()
    ...
    print(f" - Blanc (B): {victoires_b} victoires")
    print(f" - Noir  (N): {victoires_n} victoires")
    print(f" - Temps total: {end-start:.2f}s")

if __name__ == "__main__":
    benchmark_ia_vs_ia(nb_parties=10, sim_b=3, sim_n=1)
