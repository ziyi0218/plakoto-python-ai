...
if __name__ == "__main__":
    ns_human = 0        # fixed brainless side
    ns_ai = 1000         # adjustable Monte Carlo budget
    first_player = "B"  # B plays second (AI), N plays first (brainless)
    n_games = 5

    print(f"Benchmark: first={first_player}(ns={ns_human}), second={'B' if first_player=='N' else 'N'}(ns={ns_ai}), games={n_games}")
    per_turn_times: List[float] = []

    for i in range(1, n_games + 1):
        plateau = init_plateau()
        start = time.perf_counter()
        winner, turns = match_ai_vs_ai(plateau, first_player, ns_human, ns_ai, max_turns=250)
        elapsed = time.perf_counter() - start
        avg_turn = elapsed / turns if turns > 0 else 0.0
        per_turn_times.append(avg_turn)

        print(f"[Game {i:03d}] Winner={winner!r}, Time={elapsed:.3f}s, Turns={turns}, Avg/turn={avg_turn:.4f}s")

    overall_avg = sum(per_turn_times) / len(per_turn_times)
    print("\nOverall average time per turn: {:.4f}s".format(overall_avg))
