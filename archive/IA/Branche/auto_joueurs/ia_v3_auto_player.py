...
def generate_macro_moves(board, dice, current_player):
   ...
   #Cas les plus faciles
    ...
    #Cas ou aucune pièce puisse etre bougée
    if not movable:
        sig = board_signature(board)
        result = {sig: [[]]}
        _macro_moves_cache[key] = result
    ...
    used = set() #Permet d'éviter les doublons

    for i, d in enumerate(dice):
        remaining_dice = dice[:i] + dice[i+1:]

        for origine in movable:
            if current_player == "B":
                destination = origine + d
                if in_home and destination > 23:
                    destination = 24
            else:
                destination = origine - d
                if in_home and destination < 0:
                    destination = -1
            ...
            subsequent = generate_macro_moves(new_board, remaining_dice, current_player)

            for sig, moves_list in subsequent.items():
                macro_moves.setdefault(sig, []).extend([[move] + m for m in moves_list])

    _macro_moves_cache[key] = macro_moves
    return macro_moves
...
def main_auto():
    ...
    max_turns = 100       # Nombre maximum de tours autorisés
    turn_count = 0
