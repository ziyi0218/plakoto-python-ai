...
DEFAULT_SAVE_FILE = "savegame.pickle"
...
def sauvegarder_partie(plateau, current_player, dice, chemin_fichier=DEFAULT_SAVE_FILE):
    ...
    with open(chemin_fichier, "wb") as f:
        pickle.dump(etat, f)
    print(f"Partie sauvegardée (pickle) dans {chemin_fichier} ✅")
...
def charger_partie(chemin_fichier=DEFAULT_SAVE_FILE):
    ...
    dice           = etat["dice"]
    print(f"Partie chargée (pickle) depuis {chemin_fichier} ✅")
...
def determiner_premier_joueur():
    ...
    while True:
        ...
        print(f"Jet de N : {roll_N}  |  Jet de B : {roll_B}")
        if roll_N > roll_B:
            print("N commence !\n")
            return "N"
        elif roll_B > roll_N:
...
def main():
    ...
    while mode not in ('1','2'):
        mode = input("选择模式：1) 玩家 vs 玩家    2) 玩家 vs AI  > ").strip()
    ai_mode = (mode == '2')

    if ai_mode:
        # 选择难度
        lvl = ''
        while lvl not in NS_LEVEL:
            lvl = input("请选择AI难度 [L]ow/[M]edium/[H]igh > ").strip().lower()
        ns = NS_LEVEL[lvl]
        human_player = 'B'
        ai_player = 'N'
        print(f"\n人机对战：您 = {human_player}, AI = {ai_player}，模拟次数 = {ns}\n")
    else:
        human_player = ai_player = None  # 不用
    
    # ——— 2. 新游戏或载入 ———
    choix = input("N) Nouvelle partie  /  C) Charger une partie existante > ").strip().upper()
    if choix == "C":
        try:
            plateau, current_player, dice = charger_partie()
        except (FileNotFoundError, pickle.PickleError):
            print("Aucun fichier valide, démarrage d'une nouvelle partie.")
            plateau = init_plateau()
            current_player = determiner_premier_joueur()
            dice = []
    else:
        plateau = init_plateau()
        current_player = determiner_premier_joueur()
    ...
    while True:
        # — AI 回合 ——
        if ai_mode and current_player == ai_player:
            print(f"\n=== AI ({ai_player}) 的回合，模拟次数 = {ns} ===")
            # AI 一次性掷骰
            dice = lance_de()
            print("AI 投骰子结果：", dice)
            # 生成所有宏步候选并选出最优
            macro = generate_macro_moves(plateau, dice, ai_player)
            candidates = [mv for seqs in macro.values() for mv in seqs]
            if candidates:
                best_seq = monte_carlo_select(plateau, ai_player, dice, candidates, ns)
                print("AI 选择的步数：", best_seq)
                # 执行选定的宏步
                for origine, dest, dval in best_seq:
                    deplacer_pion(plateau, origine, dest, ai_player)
                    dice.remove(dval)
            else:
                print("AI 无可执行步，跳过回合。")
                dice = []
            ...
            afficher_plateau(plateau)
            if check_victoire(plateau, ai_player):
                print("AI 获胜！")
                break
        ...
        cmd = input(f"\nEntrez une commande pour le joueur {current_player} : ").strip().lower()
        ...
        if not continuer:
            if input("Voulez-vous sauvegarder avant de quitter ? (O/N) > ").strip().upper() == "O":
                sauvegarder_partie(plateau, current_player, dice)
            print("Au revoir !")
            break

        # 如果刚掷过骰子，先检查是否有合法操作可做
        if cmd == "lance":
