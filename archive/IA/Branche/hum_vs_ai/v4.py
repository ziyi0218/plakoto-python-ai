...
def main():
    print("=== Plakoto: 人机对抗模式 ===")

    # 选择难度
    while True:
        level = input("请选择难度 [L]ow, [M]edium, [H]igh > ").strip().lower()
        if level in NS_LEVEL:
            ns = NS_LEVEL[level]
            break
        else:
            print("输入无效，请输入 L、M 或 H。")
    print(f"已选择难度：{level.upper()}，AI 模拟次数 = {ns}\n")

    # 掷骰子决定先手
    print("决定先手：玩家 (B) 与 AI (N) 各掷一次骰子，点数大者先手")
    current = determiner_premier_joueur()
    print(f"先手：{'玩家' if current=='B' else 'AI'} ({current})\n")
    ...
    while True:
        ...
        if current == 'B':
            if not dice:
                print("请先投骰子: 输入 'lance' 来投骰子。")
            cmd = input(f"\n玩家 {current} 请输入命令: ").strip().lower()
            continuer, moved, dice = gestion_commande(cmd, plateau, current, dice)
            if not continuer:
                print("游戏已退出。再见！")
                break
            if moved:
                if check_victoire(plateau, current):
                    print("恭喜！玩家获胜！")
                    break
                if not dice:
                    current = 'N'
        ...
        # AI 回合
            ...
            print(f"\nAI ({current}) 的回合，模拟次数 = {ns}")
            dice = lance_de()
            print("AI 投骰子结果：", dice)

            # 生成所有宏步候选
            macro = generate_macro_moves(plateau, dice, current)
            candidates = [mv for seqs in macro.values() for mv in seqs]

            if candidates:
                best_seq = monte_carlo_select(plateau, current, dice, candidates, ns)
                print("AI 选择的步数：", best_seq)
                # 执行步数
                for o, dest, d in best_seq:
                    deplacer_pion(plateau, o, dest, current)
                    try:
                        dice.remove(d)
                    except ValueError:
                        pass
