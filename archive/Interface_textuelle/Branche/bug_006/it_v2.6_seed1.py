...
# alors qu'il aurait fallu avoir 24 - origine
...
input_file = open(r"D:\Code\Python\S4_L2N1\Interface textuelle\Branche\bug_006\input_bug006.txt", "r")
original_input = builtins.input
...
def existe_pion_plus_lointain(plateau, joueur, origine):
    ...
    for i in range(0, origine) if joueur == 'B' else range(origine + 1, 24):
        if plateau[i] and joueur in plateau[i]:
            return False
    return True
...
def generate_macro_moves(board, dice, current_player):
    ...
    is_bearing_off = all_in_home(board, current_player)
    direction = 1 if current_player == 'B' else -1

    for origine in movable:
        for d in dice:
            # Calcul de destination standard
            destination = origine + d * direction
            # Cas bearing off ?
            if is_bearing_off:
                if current_player == "B" and destination > 23:
                # On suppose destination=24 => c'est un move off
                    destination = 24
                if current_player == "N" and destination < 0:
                    destination = -1
            ...
            for sig, moves_list in generate_macro_moves(new_board, new_dice, current_player).items():
                macro_moves.setdefault(sig, []).extend(
                    [[(origine, destination, d)] + moves for moves in moves_list]
                )
    return macro_moves
...
def filter_max_pratique(macro_dict):
    ...
    for sig, coups in macro_dict.items():
        ...
        for coup in coups:
            s = score_macro_move(coup)
            if s == max_score_global and coup not in best_coups:
    ...
    for sig, moves_list in macro_moves.items():
        for move_seq in moves_list:
            ...
            for i in range(len(move_seq)):
                if move_seq[i][1] == 24 or move_seq[i][1] == -1:
                    copy_move_seq = (move_seq[0][0], move_seq[0][1], move_seq[0][1] - player_move[0])
                    if copy_move_seq == player_move:
                        return True
    return False
