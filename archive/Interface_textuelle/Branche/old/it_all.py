...
''' # Organisation des macro-coups
    macro_coups = {
        "Sortie de pions" : [(d, d) for d in des] if all(case in range(18, 24) for case in range(24) if plateau[case] and plateau[case][0] == joueur) else [],
        "Blocage" : [(d1, d2) for d1 in des for d2 in des if d1 != d2 and 
                     verification_deplacement(plateau, joueur, case_depart, case_depart + d1 + d2 if joueur == 1 else case_depart - d1 - d2, des)],
        "Déplacement stratégique" : micro_coups  
    }'''
# relire le code, pas compris cette partie TT 

# Avant Jeudi midi 13/03----faut finir:

# 1.Ajouter la partie VERIFICATION DE DEPLACEMENT ok
# 2.Ajouter la partie DEPLACEMENT DE PION ok
# 3.Ajouter la partie MAIN ok

#12/03

# 1. Ajouter Input
# 2. Remplacer micro/macro coups

#!!!!!!!!!!!!! IMPORTANT !!!!!!!!!!!!!!
# REGLE DU JEU : est-ce que on veut "Sortie de pions" (faut discuter avec les membres)
...
############################ CONVERSION DU PLATEAU POUR AFFICHAGE ############################
def convert_board_for_ascii(plateau):
    """
    Convertir le plateau de type dict[int, list[str]] en une liste de 24 éléments
    où chaque élément est [nombre_de_pions, joueur].
    On mappe "B" -> 1 et "N" -> 2.

    Entrée:
      - plateau: dict[int, list[str]]
    Sortie:
      - list[list]: Une liste de 24 éléments, chacun étant [nombre_de_pions, joueur]
    """
    ascii_board = []
    for i in range(24):
        cell = plateau[i]
        count = len(cell)
        if count == 0:
            ascii_board.append([0, None])
        else:
            player_char = cell[-1]
            player = 1 if player_char == "B" else 2 if player_char == "N" else None
            ascii_board.append([count, player])
    return ascii_board

############################ AFFICHAGE DU PLATEAU ############################################
def afficher_plateau(plateau):
    """
    Afficher le plateau en utilisant le nouveau style ASCII (PLAKOTO).
    ...
    """
    ascii_board = convert_board_for_ascii(plateau)
    afficher_plateau_ascii(ascii_board)


############################ AFFICHAGE DU PLATEAU (STYLE PLAKOTO) #############################
def afficher_plateau_ascii(plateau):
    print("\n" + "=" * 49)
    ...
    for i in range(6, -1, -1):
        ...
        for case in reversed(haut):
            ...
            symbol = 'B' if joueur == 1 else 'N' if joueur == 2 else ' '
            if i == 6 and nb > 5:
